...
window.ViewerCannon = class ViewerCannon {
  ...
  _updateTimer() {
    ...
    g.clear();
    // 背景圆
    g.circle(0, 0, r).fill({ color: 0x000000, alpha: 0.65 });
    // 灰色底弧
    g.arc(0, 0, arcR, 0, Math.PI * 2).stroke({ color: 0x333333, width: 3.5, alpha: 0.5 });
    // 进度弧 (PixiJS v8: 用 arc 直接画弧线描边)
    if (ratio > 0.01) {
      const startA = -Math.PI / 2;
      const endA   = startA + Math.PI * 2 * ratio;
      g.arc(0, 0, arcR, startA, endA).stroke({ color, width: 3.5, alpha: 0.95 });
    }

    this._timerText.text  = String(Math.ceil(this._timeLeft));
    this._timerText.style = {
      fill: color === 0xFF2200 ? '#FF2200' : '#ffffff',
